...
export const GET = (event) =>
	handle(async () => {
		...
		let messages = musestream.chatAfter(stream.id, after);
		if (!messages.length && wait > 0) {
			await new Promise<void>((resolve) => {
				const timer = setTimeout(done, wait * 1000);
				const off = musestream.hub.on(stream.id, (e) => {
			...
			messages = musestream.chatAfter(stream.id, after);
		}
...
export const POST = (event) =>
	handle(async () => {
		...
		return json({ message: view(musestream.agentChat(agent, text)) }, { status: 201 });
	});
