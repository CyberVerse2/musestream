...
import { charts, ethPrice, lurkk } from '$lib/server/app';
...
export const GET = ({ params, url }) =>
	handle(async () => {
		...
		if (!(interval in INTERVALS)) {
			throw new LurkkError(400, 'invalid', 'interval must be 1m, 5m, 1h, or 1d.');
		}
		if (!charts) throw new LurkkError(503, 'no_chain', 'Coins are not available on this server.');
